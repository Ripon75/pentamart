...
@once
    ...
    <script>
        ...
        var percent     = '{{ request()->get('percent') ?? '' }}';
        var subCategory = '{{ request()->get('sub_category') ?? '' }}';
        var route     = "{{ route('offers.categories.products', $slug) }}?page={{ $products->currentPage() }}";
        ...
        if (percent) {
            route = `${route}&percent=${percent}`;
        }

        if (subCategory) {
            route = `${route}&sub_category=${subCategory}`;
        }
    ...
    <script>
        var onScrollProductRoute = "{{ route('offers.categories.products', [$slug, 'true']) }}";
