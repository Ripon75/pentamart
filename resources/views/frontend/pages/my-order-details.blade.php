                    ...
                    <div class="bg-white">
                        <div class="p-2 md:p-6">
                            <div class="flex-wrap md:flex justify-between">
                                <div>
                                    <div>
                                        <span class="text-sm font-medium">Order ID:</span> <span
                                            class="text-sm">{{ $order->id }}</span>
                                    </div>
                                    <div>
                                        <span class="text-sm font-medium">Order Date:</span> <span
                                            class="text-sm">{{ $order->created_at }}</span>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <span class="text-sm font-medium">Total Items:</span> <span
                                            class="text-sm">{{ $order->items->count() ?? null }}</span>
                                    </div>
                                </div>
                                <div class="mt-2 md:mt-0 text-left md:text-right">
                                    {{-- <div class="font-bold">Delivery Address</div> --}}
                                    <div>
                                        <span class="text-sm font-medium">Name:</span> <span
                                            class="text-sm">{{ $order->user->name ?? null }}</span>
                                    </div>
                                    <div>
                                        <span class="text-sm font-medium">Phone:</span> <span
                                            class="text-sm">{{ $order->user->phone_number ?? null }}</span>
                                    </div>
                                    <div class="flex flex-col text-sm">
                                        <span class="w-full md:w-60 font-medium">Address: <span
                                                class="font-normal">{{ $order->shippingAddress->address ?? null }}</span></span>
                                        <span class="font-medium">Area: <span
                                                class="font-normal">{{ $order->shippingAddress->area->name ?? null }}</span></span>
                                    </div>
                                </div>
                            </div>
                            {{-- Items list --}}
                                ...
                                <table class="w-full">
                                    ...
                                    <tbody>
                                        @foreach ($order->items as $key => $item)
                                            <tr class="border border-black text-sm">
                                                <td width="40px" class="border-black border text-center">
                                                    {{ ++$key }}</td>
                                                <td width="auto"
                                                    class="text-xs md:text-sm border-black border text-left font-medium pl-1">
                                                    {{ $item->name }}
                                                </td>
                                                @php
                                    ...
                                    <tfoot>
                                        <tr class="text-sm border border-black">
                                            ...
                                            <td class="font-medium border border-black pl-1 text-right pr-2" colspan="3">
                                                Total</td>
                                            <td class="font-medium border border-black pl-1 text-right pr-1">
                                                {{ number_format($order->mrp, 2) }}</td>
                                            <td class="font-medium border border-black pl-1 text-right pr-1">
                                                {{ number_format($order->discount, 2) }}</td>
                                            <td class="font-medium border border-black pl-1 text-right pr-1">
                                                {{ number_format($order->sell_price, 2) }}</td>
                                        </tr>
                                    </tfoot>
                                </table>
                                        ...
                                        <table class="w-full mt-8">
                                            <tfoot class="">
                                                ...
                                                @if ($order->coupon)
                                                    <tr class="text-sm text-right">
                                                        <td class="" colspan="4"></td>
                                                        <td class="border border-black pr-1 font-medium">Coupon Discount
                                                        </td>
                                                        <td class="border pr-1 border-black">
                                                ...
                                                <tr class="text-sm text-right">
                                                    ...
                                                    @if ($order->is_paid)
                                                        <td class="border-b border-r pr-2 bg-gray-300 font-bold"> 0
                                                            {{ $currency }}</td>
                                                    @else
