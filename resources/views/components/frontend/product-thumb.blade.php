@if ($type === 'default')
    <div data-aos="fade-up" data-aos-anchor-placement="top-bottom"
        class="product-thumb block h-full bg-white rounded-md transition duration-150">
        <div class="rounded-md border-2 h-full">
            <div class="relative">
                ...
                <div class="thumb-image-wrapper aspect-w-1 aspect-h-1">
                    @if ($product->img_src)
                        <a class="p-2 sm:p-2 md:p-6 lg:p-6 xl:p-8 2xl:p-8"
                            href="{{ route('products.show', [$product->id, $product->slug]) }}">
            ...
            <div class="content px-3 py-2 bg-gray-50 rounded-b-md flex-1">
                ...
                @if ($product->name)
                    <a href="{{ route('products.show', [$product->id, $product->slug]) }}"
                        class="block h-5 font-medium text-primary text-sm line-clamp-2" title="{{ $product->name }}">
                        {{ $product->name }}
                    </a>
                @else
                    <div class="h-5"></div>
                ...
                @if ($product->category_id)
                    <a href="{{ route('category.page', [$product->category_id, $product->category->slug ?? '']) }}"
                        class="block h-4 line-clamp-1 text-gray-600 text-xs font-medium italic"
                ...
                <div
                    ...
                    @if ($product->offer_price > 0)
                        ...
                        <span id="header-product-mrp-label-{{ $product->id }}"
                            class="line-through text-gray-500 self-end">
        ...
        <a href="#" class="block">
            <div class="flex items-center">
                <div class="thumb-image-wrapper w-28 h-28 p-2">
                    <img class="w-full h-full object-center object-cover" src="{{ $product->img_src }}" />
                </div>
                <div class="content px-3 py-2 flex-1">
                    {{-- Show brand name --}}
                    @if ($product->brand_id)
                        <a href="{{ route('brand.page', [$product->brand_id, $product->brand->slug ?? '']) }}"
                            class="block text-xs text-gray-400">{{ $product->brand->name }}</a>
                    @endif
                    {{-- Show product name --}}
                    <a href="{{ route('products.show', [$product->id, $product->slug]) }}"
                        class="block text-primary text-base md:text-sm lg:text-sm xl:text-base font-medium">
                        {{ $product->name }}
                    </a>
                    ...
                    @if ($product->category_id)
                        <a href="{{ route('category.page', [$product->category_id, $product->category->slug]) }}"
                            class="block text-gray-600 text-xs font-medium italic">
    ...
    <div class="block border rounded-md shadow bg-gray-50">
        <div class="product-thumb">
            <div class="aspect-w-1 aspect-h-1">
                <img class="rounded-t-md w-full h-full object-center object-cover" src="{{ $product->img_src }}">
            </div>
